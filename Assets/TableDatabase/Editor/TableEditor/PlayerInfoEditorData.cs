...
{
    [MenuItem("Table/Tables/PlayerInfo", priority = 30)]
    static void CreateTable()
    {
        EditorWindow.GetWindowWithRect<PlayerInfoDataEditor>(new Rect(100, 100, 800, 500), false, "PlayerInfo数据");
    }
    ...
    private Dictionary<int, PlayerInfo> _searchDataDic;

    private PlayerInfo _tempData;
    ...
    private Dictionary<int, int> _primaryKeyDataDic;
    ...
    private void ShowDataList()
    {
        ...
        if (GUILayout.Button("添加", "OL Plus"))
        {
            ...
            if (!_primaryKeyDataDic.ContainsKey(data.Id))
            {
                _primaryKeyDataDic.Add(data.Id, 1);
            }
            else
            {
                _primaryKeyDataDic[data.Id]++;
            }
        ...
        if (_isSearch)
        {
            foreach (KeyValuePair<int, PlayerInfo> item in _searchDataDic)
            {
			    PlayerInfo data = item.Value;
                if (_tempData == data)
                {
                ...
                if (GUILayout.Button("选择", "OL Title", GUILayout.Width(50)))
                {
    ...
    private void ShowDataInfo()
    {
        ...
        if (_primaryKeyDataDic.ContainsKey(_tempData.Id))
        {
            if (_primaryKeyDataDic[_tempData.Id] > 1)
            {
                GUI.color = Color.red;
            }
        }
        EditorGUILayout.PropertyField(_tempSerializedProperty.FindPropertyRelative("Id"), new GUIContent("Id"), true);
        GUI.color = Color.white;

        EditorGUILayout.PropertyField(_tempSerializedProperty.FindPropertyRelative("PlayerName"), new GUIContent("PlayerName"), true);
        GUI.color = Color.white;

        GUILayout.BeginHorizontal();
        EditorGUILayout.PropertyField(_tempSerializedProperty.FindPropertyRelative("Icon"), new GUIContent("Icon"), true);
        if (_tempData.Icon != null)
        {
            GUILayout.Label(new GUIContent(image: _tempData.Icon.texture), GUILayout.Width(65), GUILayout.Height(64));
        }
        GUILayout.EndHorizontal();
        GUI.color = Color.white;
        ...
        EditorGUILayout.PropertyField(_tempSerializedProperty.FindPropertyRelative("Rot"), new GUIContent("Rot"), true);
        GUI.color = Color.white;
        ...
        GUILayout.EndScrollView();
		_primaryKey = _tempData.Id;
        _serializedObject.ApplyModifiedProperties();

        if (_primaryKey != _tempData.Id)
        {
            _primaryKeyDataDic[_primaryKey]--;
            if (_primaryKeyDataDic.ContainsKey(_tempData.Id))
            {
                _primaryKeyDataDic[_tempData.Id]++;
            }
            else
            {
                _primaryKeyDataDic.Add(_tempData.Id, 1);
            }
        }
        GUILayout.EndArea();
    }
    ...
    private void OnEnable()
    {
        ...
        _searchDataDic = new Dictionary<int, PlayerInfo>();
        _primaryKeyDataDic = new Dictionary<int, int>();
        ...
        for (int i = 0; i < _serializeData.DataList.Count; i++)
        {
            int key = _serializeData.DataList[i].Id;
